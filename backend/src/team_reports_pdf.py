...
def build_team_reports_pdf(reports: list[dict]) -> bytes:
    ...
    for i, row in enumerate(reports):
        ...
        if kind == "setter":
            ...
            _emit(pdf, "YouTube - chats", row.get("chats_youtube"))
            _emit(pdf, "YouTube - conv. reales", row.get("conversaciones_youtube"))
            _emit(pdf, "YouTube - calendlys", row.get("links_enviados_youtube"))
            _emit(pdf, "YouTube - agendas", row.get("agendas_youtube"))
            _emit(pdf, "Ads - chats", row.get("chats_ads"))
            _emit(pdf, "Ads - conv. reales", row.get("conversaciones_ads"))
            _emit(pdf, "Ads - calendlys", row.get("links_enviados_ads"))
            _emit(pdf, "Ads - agendas", row.get("agendas_ads"))
            _emit(pdf, "YouTube directo - agendas", row.get("agendas_youtube_directo"))
